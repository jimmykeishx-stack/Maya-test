...
function mapAdminProperty(row: PropertyRow): AdminProperty {
  ...
  return {
    ...
    areaSqft: row.area_sqft ?? row.size_sqft,
    amenities,
    featured: row.featured,
    status: normalizeStatus(row.status ?? row.market_status),
...
export async function getAdminProperties({
  ...
  if (trimmedSearch) {
    ...
    query = query.or(
      `title.ilike.%${escaped}%,slug.ilike.%${escaped}%,location.ilike.%${escaped}%,location_label.ilike.%${escaped}%,property_type.ilike.%${escaped}%`
    );
  }
...
export async function createAdminProperty(payload: AdminPropertyPayload, createdBy: string) {
  ...
  const insertPayload: PropertyInsert = {
    ...
    size_sqft: parsed.areaSqft ?? null,
