...
begin
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'properties_size_non_negative'
      and conrelid = 'public.properties'::regclass
  ) then
    alter table public.properties add constraint properties_size_non_negative check (size_sqft is null or size_sqft >= 0);
  end if;
...
create index if not exists idx_properties_listing_segment_status on public.properties (listing_type, segment, market_status);
