...
function buildMetrics(row: Record<string, unknown>) {
  ...
  const sizeSqft = readNullableNumber(row, ["area_sqft", "area_sqft", "sqft"]);
...
function mapProperty(row: PropertyRecord): Property {
  ...
  const location = readString(row, ["location", "area", "city", "location"], "Westlands");
  ...
  const size = readNumber(row, ["area_sqft", "area_sqft", "sqft"]);
