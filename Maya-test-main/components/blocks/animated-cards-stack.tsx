...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      dark:
        "flex size-full flex-col items-center justify-center gap-6 rounded-[1.75rem] border border-[rgba(212,173,94,0.18)] bg-[#171411]/86 p-6 text-white backdrop-blur-md",
      light:
        "flex size-full flex-col items-center justify-center gap-6 rounded-[1.75rem] border border-black/8 bg-[rgba(255,251,244,0.86)] p-6 backdrop-blur-md"
    }
  },
  defaultVariants: {
    variant: "light"
...
export const ContainerScroll: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ children, style, className, ...props }) => {
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={scrollRef}
        className={cn("relative min-h-[260svh] w-full", className)}
