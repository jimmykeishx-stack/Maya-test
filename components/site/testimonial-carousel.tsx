...
export function TestimonialCarousel() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setActiveIndex((current) => (current + 1) % clientReviews.length);
    }, 5000);

    return () => window.clearInterval(interval);
  }, []);

  const active = clientReviews[activeIndex];
  ...
  return (
    <section className="section-space">
      <div className="site-shell grid gap-8 rounded-[2.2rem] border border-black/6 bg-[rgba(255,251,244,0.76)] px-6 py-10 shadow-[0_24px_80px_rgba(15,12,8,0.06)] md:grid-cols-[0.7fr_1.3fr] md:px-10">
        <div>
          <p className="quiet-label text-[var(--gold-strong)]">What Our Clients Say</p>
